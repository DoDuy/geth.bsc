...
// TraceCallMayny is like TraceCall but it tracks dependent transactions sequentially
func (api *API) TraceCallMany(ctx context.Context, txs []ethapi.TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceCallConfig) ([]*txTraceResult, error) {
	// Try to retrieve the specified block
	var (
		err     error
		block   *types.Block
	...
	for i, tx := range txs {
		...
		res, err := api.traceTx(ctx, msg, txctx, vmctx, statedb, traceConfig, false)
		if err != nil {
